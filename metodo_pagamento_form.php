...
if($id){
    $stmt = $pdo->prepare("SELECT nome FROM METODO_PAGAMENTO WHERE id=?");
    $stmt->execute([$id]);
...
if($_SERVER['REQUEST_METHOD']==='POST'){
    $nome = $_POST['nome'] ?? '';
    if($id){
        $stmt = $pdo->prepare("UPDATE METODO_PAGAMENTO SET nome=? WHERE id=?");
        $stmt->execute([$nome,$id]);
    }else{
        $stmt = $pdo->prepare("INSERT INTO METODO_PAGAMENTO (nome) VALUES (?)");
        $stmt->execute([$nome]);
        $id = $pdo->lastInsertId();
    }
    header('Location: metodo_pagamento_list.php');
    exit();
}

$pageTitle = $id ? 'Editar Método de Pagamento' : 'Novo Método de Pagamento';
include 'header.php';
?>
