...
$produtos = $pdo->query("SELECT ID,NOME,VALOR_UNITARIO FROM PRODUTO p JOIN CATEGORIA_PRODUTO c ON p.ID_CATEGORIA=c.ID WHERE UPPER(c.NOME) IN ('PECA','PEÇAS','PEÇA','PECAS') ORDER BY NOME")->fetchAll(PDO::FETCH_ASSOC);
