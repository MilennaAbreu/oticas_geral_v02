...
$error = '';
if($id){
  $stmt = $pdo->prepare("SELECT id, nome FROM MARCA_PRODUTO WHERE id=?");
  $stmt->execute([$id]);
...
if($_SERVER['REQUEST_METHOD']==='POST'){
  ...
  try {
    if($id){
      $stmt = $pdo->prepare("UPDATE MARCA_PRODUTO SET nome=? WHERE id=?");
      $stmt->execute([$nome, $id]);
    } else {
      $stmt = $pdo->prepare("INSERT INTO MARCA_PRODUTO (nome) VALUES (?)");
      $stmt->execute([$nome]);
    }
    header('Location: marcas_list.php');
    exit;
...
$pageTitle = $id ? 'Editar Marca' : 'Nova Marca';
include 'header.php';
?>
